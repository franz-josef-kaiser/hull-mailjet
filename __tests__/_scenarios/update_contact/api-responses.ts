...
import { API_BASE_URL, MJ_IDENT1_EMAIL, AUTH_HEADER_CHECK, MJ_IDENT1_ID, HULL_USER_NAME1 } from "../../_helpers/constants";
...
const setupApiMockResponses = (nockFn: (basePath: string | RegExp | Url, options?: nock.Options | undefined) => nock.Scope): IApiResponseNocked[] => {
    ...
    // PUT Contact
    const responseBody2 = _.cloneDeep(ApiresponseV3ContactPut);
    responseBody2.Data[0].ID = MJ_IDENT1_ID;
    responseBody2.Data[0].Email = MJ_IDENT1_EMAIL;
    responseBody2.Data[0].Name = HULL_USER_NAME1;
    const url2 = `${API_BASE_URL}/v3/REST/contact/${MJ_IDENT1_EMAIL}`;

    nockFn(`${API_BASE_URL}`)
        .matchHeader('Authorization', AUTH_HEADER_CHECK)
        .put(`/v3/REST/contact/${MJ_IDENT1_EMAIL}`)            
