...
const setupExpectations = (ctx: ContextMock, apiResponses: IApiResponseNocked[]) => {
    // Check that settings.update has been called
    expect(ctx.client.utils.settings.update.mock.calls).toHaveLength(1);
    expect(ctx.client.utils.settings.update.mock.calls[0]).toEqual([{
        incoming_eventcallbackurl_eventtypes: []
    }]);
    // Check logged metrics
    const deletedEventTypes: MailJetEventType[] = [
        "sent",
        "unsub", // DOES have a different host, so goes last!
    ];
