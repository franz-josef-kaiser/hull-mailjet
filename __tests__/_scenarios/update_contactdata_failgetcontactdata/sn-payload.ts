...
import { API_KEY, API_SECRET_KEY, CONCONF_SUBACCOUNT_SLUG, HULL_SEGMENT1, MJ_LIST1, HULL_USER_NAME1, HULL_USER_JOBTITLE1, CONCONF_OUTBOUND_ATTRIBS } from "../../_helpers/constants";
...
const configurePayload = (): IHullUserUpdateMessage => {
    ...
    // Configure the custom mappings
    _.set(basePayload, "connector.private_settings.contact_attributes_outbound", CONCONF_OUTBOUND_ATTRIBS);

    // Configure the unified_data/job_title
    _.set(basePayload, "messages[0].user['traits_unified_data/job_title']", HULL_USER_JOBTITLE1);

    // Ensure that the user is in the configured segment
    _.set(basePayload, "messages[0].segments", [ HULL_SEGMENT1 ]);
